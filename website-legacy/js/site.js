/**
 * Nature Ninjas site.js — config-driven nav, pages, and partials
 */
(function () {
  const cfg = () => window.NATURE_NINJAS_CONFIG;
  if (!cfg()) {
    console.error('NATURE_NINJAS_CONFIG not loaded. Include js/config.js before site.js');
  }
...
<footer class="site-footer">
    ...
    <div class="site-footer__grid">
      <div>
        ...
        <figure class="site-footer__qr">
          <img src="images/qr/ghoulhunters-website-qr.png" alt="QR code linking to the Nature Ninjas website with community invasive species watch information" width="200" height="200" loading="lazy">
          <figcaption>Scan to visit and share the Nature Ninjas website.</figcaption>
        </figure>
      </div>
      <div>
  ...
  function initWhoWeArePage() {
    ...
    if (about && school && suburb) {
      about.innerHTML = `<strong>Nature Ninjas</strong> is a <strong>${escapeHtml(level)}</strong> community project from <strong>${escapeHtml(school)}</strong> in <strong>${escapeHtml(suburb)}</strong>. We run invasive species watch programs across our region and help neighbours protect <strong>nature</strong> and <strong>biodiversity</strong> by sharing what to look for and how to report safely.`;
    }
